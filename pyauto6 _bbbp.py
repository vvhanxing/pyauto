...
def ACD_save_mol(save_mol_name,SMILES_str):
    dlg_spec = app.window(title=r'ACD/ChemSketch (Freeware) - [C:\Users\Administrator\Desktop\hubChem\lstm_mol\BBBP_file\0000.sk2]')
...
floder = "C:/Users/Administrator/Desktop/hubChem/lstm_mol/BBBP/"
...
for count ,file in enumerate(files[60:120]):
    print(count ,file)
    with  open( floder+ file,"r") as f:
        SMILES_str = f.readlines()[0]
        print(SMILES_str )
    if "." not in SMILES_str and "Cu" not in SMILES_str and "Hg" not in SMILES_str and "Pt" not in SMILES_str and "Bi" not in SMILES_str and "Mo" not in SMILES_str and "Fe" not in SMILES_str and "Ir" not in SMILES_str and  "M" not in SMILES_str:
